...
void compute_density_combined(ParticlesSoA& p, const SPHSettings& s) {
    const int n = p.n;
...
void compute_forces_combined(ParticlesSoA& p, const SPHSettings& s) {
    const int n = p.n;
    const float* px = p.pos_x.data();
    const float* py = p.pos_y.data();
    ...
    for (int i = 0; i < n; i++) {
        ...
        for (; j + 4 <= n; j += 4) {
            ...
            invRLen = vmulq_f32(invRLen, vrecpsq_f32(rLen, invRLen));
...
int main() {
    ...
    InitWindow(WINDOW_WIDTH, WINDOW_HEIGHT, "SPH Phase 4 - Combined SIMD+OpenMP");
    SetTargetFPS(60);
    ...
    while (!WindowShouldClose()) {
        ...
        frameCount++;
        DrawFPS(10, 10);
        DrawText(TextFormat("Particles: %d  [Combined SIMD+OpenMP %d threads]", numParticles, numThreads), 10, 30, 20, BLACK);
    ...
    // ----- Session log -----
    {
        ...
        fname << "benchmarks/sph_combined_" << std::put_time(tm, "%Y%m%d_%H%M%S") << ".log";
        ...
        if (log.is_open()) {
            log << "SPH Phase 4 - Combined SIMD+OpenMP Session Log\n";
            log << "===============================================\n";
